...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(num_hidden_layers):
            ...
            if self.use_batchnorm:
                X_last_layer, current_cache = affine_bn_relu_forward(X_last_layer, 
                ...
                X_last_layer, current_cache = affine_relu_forward(X_last_layer,    
                                                                  self.params['W' + str(i+1)], 
                                                                  self.params['b' + str(i+1)])
            cache_history.append(current_cache)
            
            if self.use_dropout:
                X_last_layer, current_cache = dropout_forward(X_last_layer, self.dropout_param)
                cache_history.append(current_cache)
                
            regularization += np.sum(self.params['W' + str(i+1)]**2)
        X_last_layer, current_cache = affine_forward(X_last_layer, self.params['W' + str(num_hidden_layers+1)], self.params['b'+str(num_hidden_layers+1)])
        regularization += np.sum(self.params['W' + str(num_hidden_layers+1)]**2)
        cache_history.append(current_cache)
        scores = X_last_layer
        ############################################################################
        #                             END OF YOUR CODE                             #
        ...
        loss += self.reg * 0.5 * regularization
        dx , dw, db = affine_backward(dout, cache_history.pop())
        grads['W' + str(num_hidden_layers+1)] = dw + self.reg * self.params['W'+str(num_hidden_layers+1)]
        grads['b' + str(num_hidden_layers+1)] = db
        i = num_hidden_layers-1
        
        ###note that with dropout, we can't use i for cache_history indices
        while i >= 0:
            if self.use_dropout:
                dx = dropout_backward(dx, cache_history.pop())
            if self.use_batchnorm:
                dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dx, cache_history.pop())
            else:
